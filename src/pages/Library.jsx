  ...
  return (
    <div className="relative min-h-screen">
      <motion.div className="p-10 bg-background min-h-screen backdrop-blur-none">
        <h1 className="text-4xl font-header text-accent mb-6">Song Library</h1>
        ...
        <input
          ...
          className="w-full max-w-md mb-6 p-2 rounded bg-background-elev text-white font-body border border-zinc-600"
        />
        
        {(search || selectedNationalities.length > 0 || selectedMeters.length > 0) && (
          <p className="text-primary text-sm mb-4 font-body-bold">
            {resultCount} result{resultCount !== 1 ? 's' : ''} found
          </p>
        )}

        <div className="flex flex-col font-body sm:flex-row sm:gap-6">
        ...
        <div className="flex flex-wrap gap-2 mt-4 items-center relative">
          {selectedNationalities.map((n) => (
            <span key={`nat-${n}`} className="bg-secondary text-primary font-body-bold text-sm px-3 py-1 rounded-full flex items-center">
              {n}
          ...
          {(selectedNationalities.length > 0 || selectedMeters.length > 0) && (
            <button onClick={() => { setSelectedNationalities([]); setSelectedMeters([]); }} className="ml-auto bg-red-600 font-body text-white text-sm px-3 py-1 rounded-full">Clear All</button>
          )}
        </div>
